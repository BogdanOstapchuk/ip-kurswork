...
<div class="container sort-container">
    <h1>Сериалы</h1>
    <form action="/serials/" method="post" class="sort-form">
            <?php if ($_POST['years'] ?? ''): ?>
                <select name="years">
    ...
    <div class="films">
        ...
        <?php foreach ($serials as $serial):?>
            <div class="film" id="1">
                <div class="logo-film">
                    <?php if($serial['is_new'] == 1):?>
                        <p>NEW</p>
                        <?php if($serial['rating'] > 8):?>
                            <img class="green" src="<?php echo Film::getPreviewImg($serial['img_preview']);?>" alt="photo-1" />
                        <?php elseif($serial['rating'] > 6):?>
                            <img class="orange" src="<?php echo Film::getPreviewImg($serial['img_preview']);?>" alt="photo-1" />
                        <?php else:?>
                            <img class="red" src="<?php echo Film::getPreviewImg($serial['img_preview']);?>" alt="photo-1" />
                        <?php endif;?>
                    <?php else:?>
                        <?php if($serial['rating'] > 8):?>
                            <img class="green" src="<?php echo Film::getPreviewImg($serial['img_preview']);?>" alt="photo-1" />
                ...
                <div class="names-film">
                    <a href="/film/<?php echo $serial['id'];?>">
                        <p class="names-ru"><?php echo $serial['name_ru']?></p>
                        <p class="type-film"><?php echo $serial['type']?> (<?php echo $serial['year']?> год)</p>
                    </a>
                </div>
            </div>
        <?php endforeach; ?>
